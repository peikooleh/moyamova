...
(function(){
  ...
  async function activateAndReload(reg){
    ...
    try {
      ...
      // Дадим UI время отрисовать тост/оверлей перед перезапуском
  ...
  if (btn && !btn.dataset.updatesBound){
    ...
    btn.addEventListener('click', function(){
      ...
      // Показываем тост сразу (иначе при быстром перезапуске его не успевают увидеть)
      try { setToast(t('checking')); } catch(_){ }
